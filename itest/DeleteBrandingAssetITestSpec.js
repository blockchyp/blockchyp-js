  ...
  it('deletes a terminal branding asset.', function (done) {
    ...
    if (testDelay > 0) {
      var messageRequest = {
        ...
        message: 'Running DeleteBrandingAsset in ' + testDelay + ' seconds...'
      }
    ...
    setTimeout(function () {
      ...
      if (request0.uploadId) {
        uploadId = request0.uploadId
            ...
            client.updateBrandingAsset(request0).then(function (httpResponse) {
          let response = httpResponse.data
          ...
          return client.deleteBrandingAsset(request)
        })
        .then(function (httpResponse) {
          let response = httpResponse.data
