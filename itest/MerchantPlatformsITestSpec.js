...
describe('MerchantPlatforms', function () {
  ...
  it('returns all platforms configured for a gateway only merchant.', function (done) {
    ...
    if (testDelay > 0) {
      var messageRequest = {
        ...
        message: 'Running MerchantPlatforms in ' + testDelay + ' seconds...'
      }
    ...
    console.log('Running merchantPlatforms...')
    ...
    setTimeout(function () {
      ...
      if (request0.uploadId) {
        uploadId = request0.uploadId
          ...
          return client.merchantPlatforms(request)
        })
        .then(function (httpResponse) {
          let response = httpResponse.data
