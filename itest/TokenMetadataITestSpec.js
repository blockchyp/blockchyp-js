...
describe('TokenMetadata', function () {
  ...
  it('can retrieve token metadata', function (done) {
    ...
    if (testDelay > 0) {
      var messageRequest = {
        ...
        message: 'Running TokenMetadata in ' + testDelay + ' seconds...'
      }
    ...
    console.log('Running tokenMetadata...')
    ...
    setTimeout(function () {
      ...
      if (request0.uploadId) {
        uploadId = request0.uploadId
          ...
          return client.tokenMetadata(request)
        })
        .then(function (httpResponse) {
          let response = httpResponse.data
